...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  secureDeclaration(context) {
    ...
    const entityNames = this.visit(context.secureEntityList);
    ...
    if (context.relPrivilegesSecurity) {
      ...
      relPrivileges = this.visit(context.relPrivileges);
    }

    return { entityNames, excludedNames: [], securityType, roles, privileges, parentPrivileges, relPrivileges };
  }
  ...
  relPrivilegesSecurity(context) {
    return _.map(context.relPriveleges, element => this.visit(element));
  }
  ...
  roleDefinition(context) {
    ...
    if (context.rolePropList) {
      ...
      actionList = context.rolePropList[0].children.NAME.map(nameToken => nameToken.image);
    }
  ...
  privDefinition(context) {
    const action = context.NAME[0].image;
    let privList = [];
    if (context.rolePropList) {
