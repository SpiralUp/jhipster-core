...
const { convertSecure } = require('../converters/parsedJDLToJDLObject/secure_converter');
...
function fillClassesAndFields() {
  const jdlEntities = convertEntities(
    parsedContent.entities,
    getJDLFieldsFromParsedEntity,
    getJDLSecureFromParsedEntity
  );
...
function fillSecurity() {
  ...
  for (let i = 0; i < jdlSecureStmts.length; i++) {
    const secure = jdlSecureStmts[i];
    for (let j = 0; j < secure.entityNames.length; j++) {
      const entityName = secure.entityNames[j];
      const entity = jdlObject.getEntity(entityName);
      if (!entity) {
        // console.log(`ERROR: Entity not found: ${entityName}`);
      }
      entity.secure = convertSecure(secure);
    }
  }
}
