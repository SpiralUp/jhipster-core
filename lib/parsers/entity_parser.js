...
function fillEntities() {
  jdlObject.forEachEntity(entity => {
    if (entity.name.toLowerCase() !== USER.toLowerCase()) {
      ...
      setSecurityOfEntity(entity.name);
    }
  });
}
...
function setSecurityOfEntity(entityName) {
  const entity = jdlObject.getEntity(entityName);
  if (entity.secure) {
    if (entity.secure.securityType === 'roles') {
      entities[entityName].secure = {
        securityType: entity.secure.securityType,
        roles: []
      };
      for (let i = 0; i < entity.secure.roles.length; i++) {
        entities[entityName].secure.roles.push({
          role: entity.secure.roles[i].role,
          actionList: entity.secure.roles[i].actionList
        });
      }
    } else if (entity.secure.securityType === 'privileges') {
      entities[entityName].secure = {
        securityType: entity.secure.securityType,
        privileges: []
      };
      for (let i = 0; i < entity.secure.privileges.length; i++) {
        entities[entityName].secure.privileges.push({
          action: entity.secure.privileges[i].action,
          privList: entity.secure.privileges[i].privList
        });
      }
    } else if (entity.secure.securityType === 'parentPrivileges') {
      entities[entityName].secure = {
        securityType: entity.secure.securityType,
        parentPrivileges: {
          parent: null,
      ...
      entities[entityName].secure.parentPrivileges = {
        parent: entity.secure.parentPrivileges.parent,
        field: entity.secure.parentPrivileges.field
      };
    } else if (entity.secure.securityType === 'relPrivileges') {
      entities[entityName].secure = {
        securityType: entity.secure.securityType,
        relPrivileges: {
          fromEntity: null,
      ...
      entities[entityName].secure.relPrivileges = {
        fromEntity: entity.secure.relPrivileges.fromEntity,
        fromField: entity.secure.relPrivileges.fromField,
        toEntity: entity.secure.relPrivileges.toEntity,
        toField: entity.secure.relPrivileges.toField
      };
    }
  }
}
